...
logging.basicConfig(
    filename=os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'logs', 'last_run.log'),  # name of the file
...
class Simulation:
    def __init__(self, settings_file, movment_flag = True):
        ...
        log_file = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'logs', 'last_run.log')
        ...
        error_data_path = os.path.join(data_dir, 'error_data.csv')
        error_params_path = os.path.join(data_dir, 'error_params.csv')
        covariance_path = os.path.join(data_dir, 'covariance_data.csv')
        
        self.csvfile = open(error_data_path, 'w', newline='')
        self.error_file = open(error_params_path, 'w', newline='')
    ...
    def load_texture(self, image):
        ...
        if not os.path.isabs(image):
            ...
            if image.startswith('tags/'):
                # Update old tag paths to new assets/tags structure
                image_path = os.path.join(project_root, 'assets', image)
            else:
        ...
        logging.info(f"Loaded texture {image_path} with width: {width}, height: {height}")
...
if __name__ == '__main__':
    config_path = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'sim_settings.json')
    sim = Simulation(config_path, movment_flag=True)
    try:
        sim.run()
